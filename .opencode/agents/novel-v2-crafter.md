...
写完正文并更新字数后，读取项目的 `config.yaml` → `质量检查.章节最多字数`，如果实写字数超过该值，执行分章操作：

```
1. 读阈值：读取 config.yaml → 质量检查.章节最多字数（各项目不同）
...
- **建议字数** — 该场景推荐的字数上限（基于子类型和密度自动推算）。如果实写字数超过建议字数的 1.5 倍，考虑是否场景过载或需要拆分。
...
7. **写作后分章** — 先写完整内容，写完再判断是否拆分。当正文超过 config.yaml 的 `质量检查.章节最多字数` 时，必须按「正文分章」流程执行拆分
