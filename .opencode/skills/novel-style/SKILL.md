...
|模式|阶段|调用时机|产出|
|------|------|---------|------|
|**提取模式**|P-1.5|创意构思（P1）之后，用户提供参考文本时|`styles/{名称}.yaml` + 写入 `config.yaml.活跃风格`|
...
- 如果没有参考文本，跳过 P-1.5，使用内置风格（默认"通俗网文风"）
...
2. **7 个维度必须齐全**
 - *narrative_tone* → 腔调的催眠/警醒功能
 - *sentence_structure* → 词汇策略与句法选择
 - *pacing* → 节奏与角色意志的关系
 - *dialogue_style* → 腔调在对话中的变形
 - *vocabulary_register* → 措辞范围与语域
 - *rhetorical_features* → 修辞策略选择（"踩影子找影子"——识别前人腔调标记）
 - *forbidden_patterns* → 腔调边界（破绽即不一致的标记）
3. **每个维度至少一个非空字段**
4. **参考文本原文不存储**：只在提取 prompt 中分析
5. **叙事腔调适用于叙述者**，不影响角色对话的个性化声音
6. **踩影子找影子**：提取时不仅记录7维度表面特征，还需标注"这是谁的影子"和"与同谱系的区别"。腔调的根本来源不是修辞技巧，"描述程序即观察程序"。

风格存在的唯一目的：在 P8 章节写作时由 novel-writer.md 读取并注入到写作 prompt 中。

## 使用方式

### 提取模式（P-1.5：文本 → style.yaml，可选）

在 P1 创意构思之后触发。用户提供 2-3 段参考文本，Agent 通过 P-1.5 提取流程生成新风格。详细工作流见 `references/style_extraction.md`。
...
> P-1.5 是可选阶段。如果用户没有提供参考文本，跳过此阶段，使用内置风格（默认"通俗网文风"）。
...
|技能|负责|不负责|
|------|------|--------|
|novel-style|格式定义 + 提取工作流（P-1.5） + 验证工作流（P10） + 脚本维护|实际写作、实际质量检测|
|novel-chapter|写章节时遵循 prompt 中的写作约束|定义 style.yaml 格式|
|novel-quality|执行风格一致性检查（通过 prompt）|定义检查维度和标准|
|novel-writer.md|检测 活跃风格 → P-1.5 调度 → 加载注入 prompt → 调度验证（P10）|定义风格数据|
