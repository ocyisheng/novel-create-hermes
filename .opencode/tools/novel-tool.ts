...
export default tool({
  ...
  args: {
    ...
    unitType: tool.schema.string().optional().describe("单元类型过滤 (SCENE/CHARACTER_ARC 等)"),
    ...
    dryRun: tool.schema.boolean().optional().describe("试运行"),
    currentVolume: tool.schema.number().optional().describe("更新写作进度：当前卷"),
    currentChapter: tool.schema.number().optional().describe("更新写作进度：当前章"),
    volumeOutlineStatus: tool.schema.string().optional().describe("更新写作进度：卷大纲状态描述"),
    volumeOutlineDone: tool.schema.number().optional().describe("更新写作进度：已完成大纲的卷数"),
