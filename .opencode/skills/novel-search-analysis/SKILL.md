...
所有模式的最终目标一致：**确保写作前文档（P1-P7）始终符合用户想法**。

在内容生成后，提供搜索、分析、意图对齐核验能力，找出文档与用户意图之间的偏差。
**不生成新内容，不直接修改文件**——仅输出结构化分析报告（含可执行变更集），作为编排层调度 novel-edit 修正偏差的决策依据。

**分析范围：P1-P7（写作前文档）**——创意、世界观、角色、总纲、情节、分卷、分纲、设定文件、追踪文件。不涉及 P8+ 章节正文。
...
| novel-quality | 技术质量检测（写得好不好） | 写作前文档的意图对齐和结构一致性 |
...
**核心分析域为 P1-P7（写作前文档）**，这些是结构化 YAML 文件，适合精确搜索和比对。

| 优先级 | 目录 | 文件类型 | 内容说明 |
|--------|------|---------|---------|
| P1 | `characters/` | `*.yaml` | 角色档案 |
| P1 | `worldbuilding/` | `*.yaml` | 世界观设定 |
| P2 | `outline/` | `*.yaml` | 总纲/分卷/分纲/情节线/追踪（含 graph） |
...
**用途**：基于项目图谱的全量偏差检测。一次性检测 **P1-P7** 所有实体中从上次扫描后有变动的部分，合并偏差状态，输出带变更集的聚合报告。
