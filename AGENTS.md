...
而是一个"创作引擎"——以**叙事单元网络（graph）** 为核心存储，提供 9 个技能覆盖从环境安装、
...
├── opencode.json                  ← 项目级配置（已注册 9+ 个 skill）
├── README.md                      ← 用户入门指南
...
| `novel-ideation` | 创意方案生成：发散/聚焦两模式 | `task(subagent_type="novel-ideation", load_skills=["novel-ideation"], ...)` |
...
| `humanizer-zh-enhanced` | 文本去 AI 味：覆盖 27 种 AI 写作模式 | `skill("humanizer-zh-enhanced")` |

> **V2 创作路由**：所有创作任务（章节/角色/世界观/情节/总纲/大纲/编辑/质检/导出）统一走 `novel-v2-crafter`，通过 `FOCUS TYPE`、`PREHEAT LEVEL`、`CYCLE TYPE` 参数区分操作类型。详见 `novel-writer.md` V2 路由表。
>
