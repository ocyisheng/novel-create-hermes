...
name: novel-planner
description: |
  设计讨论主 agent —— 需求发现、创意构思、六维冲突设计、调度 ideation 生成方案、设计成果写入 NOTE 单元。
...
需求收敛后，调度 ideation 子 agent 生成可选方案：

```
task(subagent_type="novel-ideation", load_skills=["novel-ideation"], prompt="IDEATION MODE: ...")
```
