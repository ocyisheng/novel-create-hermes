...
def test_relations(store):
    ...
    sc = store.create_unit(UnitType.SCENE, "后山对决", "决战场景", belongs_to_chapter=5)
...
def test_weak_signals(store):
    ...
    # 创建几个有内容重叠的单元
    s1 = store.create_unit(UnitType.SCENE, "测试场景", "主角 拔剑 灵气 淬体", belongs_to_chapter=3)
    s2 = store.create_unit(UnitType.SCENE, "另一场景", "拔剑 灵气 对决", belongs_to_chapter=5)
...
def test_projection_engine(store, project_root):
    ...
    # 分纲投影
    store.create_unit(UnitType.SCENE, "第3章场景1", belongs_to_chapter=3, actor="test")
