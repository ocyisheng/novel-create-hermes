...
V2 不再有 P1-P15 线性阶段。创作是循环的——任何操作在任何时候都可以进行，数据一致性由 graph 保障。
...
用户输入
  ...
  ├─ P14 搜索分析（搜索/查找/分析/核验/对齐/整体检测）? → skill("novel-search-analysis")
...
V2 路由替代传统 P1-P15 全部阶段路由。创作操作统一映射到焦点类型：
