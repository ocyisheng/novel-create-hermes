...
读取总纲+分卷+情节线+角色列表+叙事策略+出场节奏，为每章输出分纲。
...
|出场节奏|`outline/情节线/*.yaml`|每条线的 `完整档案.角色参与.出场节奏`（角色出场规划→首次出场/关键章节/不活跃区间）|`glob` + `read`|
...
陈世骧评《天龙八部》"离奇与松散"——松散不是缺陷，是书场叙事特质的书面化。分卷/分纲设计需要注意三个平衡：

1. **间奏章节（volume.yaml已有字段）**：已有模板间的`间奏章节`字段用于标记非推进章节。填入时注意——不是"无用的内容"，而是"允许野性的空间"。
2. **微弧中的闲章**：分卷内的微弧不必每弧都推动主线。允许"无事处生事"的闲章存在——"专在无事中生事"是书场传统的美学手段。
3. **松散度的节奏控制**：张力曲线（chapter.yaml已有字段）不必追求完美抛物线。"不急欲观后文"才是书场真趣味。按章节类型区分：张力推进章追求效率，间奏章允许散漫闲笔。
